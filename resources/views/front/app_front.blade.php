...
<html lang="en">
   	<head>
        ...
        @if($route == 'front_property_result')
            @php $item_row = \App\Models\PagePropertyItem::where('id',1)->first(); @endphp
            <title>{{ $item_row->seo_title }}</title>
            <meta name="description" content="{{ $item_row->seo_meta_description }}">
        @endif

        @if($route == 'front_property_detail')
            @php
                $main_url = Request::url();
                $slug = explode('property/',$main_url);
            @endphp
            @php $item_row = \App\Models\Property::where('property_slug',$slug[1])->first(); @endphp
            <title>{{ $item_row->seo_title }}</title>
            <meta name="description" content="{{ $item_row->seo_meta_description }}">
        @endif
        ...
        @if($route == 'front_property_location_all')
            @php $item_row = \App\Models\PagePropertyLocationItem::where('id',1)->first(); @endphp
            <title>{{ $item_row->seo_title }}</title>
            <meta name="description" content="{{ $item_row->seo_meta_description }}">
        @endif

        @if($route == 'front_property_category_all')
            @php $item_row = \App\Models\PagePropertyCategoryItem::where('id',1)->first(); @endphp
            <title>{{ $item_row->seo_title }}</title>
            <meta name="description" content="{{ $item_row->seo_meta_description }}">
        @endif
        ...
        @if($route == 'front_property_location_detail')
            @php
                $main_url = Request::url();
                $slug = explode('property/location/',$main_url);
            @endphp
            @php $item_row = \App\Models\PropertyLocation::where('property_location_slug',$slug[1])->first(); @endphp
            <title>{{ $item_row->seo_title }}</title>
            <meta name="description" content="{{ $item_row->seo_meta_description }}">
        @endif

        @if($route == 'front_property_category_detail')
            @php
                $main_url = Request::url();
                $slug = explode('property/category/',$main_url);
            @endphp
            @php $item_row = \App\Models\PropertyCategory::where('property_category_slug',$slug[1])->first(); @endphp
            <title>{{ $item_row->seo_title }}</title>
            <meta name="description" content="{{ $item_row->seo_meta_description }}">
        @endif
        ...
        @if($route == 'customer_dashboard'||$route == 'customer_package'||$route == 'customer_package_purchase_history'||$route == 'customer_property_view'||$route == 'customer_property_view_detail'||$route == 'customer_property_add'||$route == 'customer_property_edit'||$route == 'customer_my_reviews'||$route == 'customer_my_review_edit'||$route == 'customer_wishlist'||$route == 'customer_update_profile'||$route == 'customer_update_password'||$route == 'customer_update_photo'||$route == 'customer_update_banner'||$route == 'customer_package_purchase_invoice'||$route == 'customer_package_purchase_history_detail')
            @php $item_row = \App\Models\PageOtherItem::where('id',1)->first(); @endphp
            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-lg-6 col-md-7 col-sm-12">
                        @if($g_setting->customer_property_option == 'On')
                        <ul class="top-right">
                            <li>
                            ...
                            <li class="currency">
                                <a href="{{ route('customer_property_add') }}" class="nav-link"><i class="fas fa-plus"></i> {{ MENU_ADD_PROPERTY }}</a>
                            </li>                            
                        </ul>
                        @endif
                    </div>
