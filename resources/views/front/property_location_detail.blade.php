...
@section('content')
    ...
    <div class="page-banner" style="background-image: url('{{ asset('uploads/page_banners/'.$property_location_page_data->banner) }}')">
        ...
        <h1>{{ PROPERTY_LOCATION_COLON }} {{ $property_location_detail->property_location_name }}</h1>
        <nav>
            <ol class="breadcrumb justify-content-center">
                <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ HOME }}</a></li>
                <li class="breadcrumb-item"><a href="{{ route('front_property_location_all') }}">{{ $property_location_page_data->name }}</a></li>
                <li class="breadcrumb-item active">{{ $property_location_detail->property_location_name }}</li>
            </ol>
        </nav>
    </div>
